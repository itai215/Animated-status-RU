...
class AnimatedStatus {
	...
	getAuthor() {
		return "toluschr & Itai";
	}

	getDescription() {
		return "Оживите свой статус в Discord";
	}
	...
	Status_Animate(index = 0) {
		...
		if (this.animation[index] == undefined) {
			BdApi.showToast("Анимированный статус: статус не установлен. Зайдите в «Настройки»> «Плагины», чтобы настроить собственную анимацию!");
			return;
		}
	...
	getSettingsPanel() {
		...
		// timeout
		settings.appendChild(GUI.newLabel("Время на ключевой кадр (в миллисекундах)"));
		...
		timeout.addEventListener("focusout", () => {
			if (parseInt(timeout.value) < this.kMinTimeout) {
				...
				BdApi.showToast(`Тайм-аут не должен быть меньше, чем ${this.kMinTimeout}`, {type: "ошибка"});
			}
		});
		...
		addStep.title = "Добавить шаг в конец";
		addStep.onclick = () => {
		...
		delStep.title = "Удалить последний шаг";
		...
		let changeEditMode = GUI.newButton("Изменить режим редактирования");
		...
		save.onclick = () => {
			...
			// Show Toast
			BdApi.showToast("Настройки сохранены!", {type: "успех"});
...
const Status = {
	...
	request: () => {
		...
		req.onload = () => {
			...
			if (err != undefined)
				BdApi.showToast(`Animated Status: Error: ${err}`, {type: "ошибка"});
		};
		return req;
	},
